...
scripts/*.py                              triage, goal, budget, journal, evidence, claims, rank, spark, report, promote, retrieve, validate_skill, fence
library/                                  versioned validated skills + registry.json + receipts.jsonl
tests/
```
Target-project output: `AGI_Research/runs/<goal_id>/{goal.json,journal.jsonl,evidence.jsonl,claims.jsonl,hypotheses.json,spark.json,fence/,FINDINGS.md,HANDOFF.md}`
...
**PR:** one.
**Depends on:** S-14. Accepted from the Proposed section 2026-09-09.
...
**Acceptance:** WHEN `gh repo view --json description,repositoryTopics` runs THEN it SHALL show the description and eight topics, and WHEN `test -f LICENSE && grep -ci 'claude code' README.md` runs THEN it SHALL print at least 1.
...
### S-28 — README says who the harness is for: frontier models
**PR:** one.
**Depends on:** S-27.
**Files:** `README.md`, `.claude-plugin/plugin.json`.
**Today:** README never says which models the plugin is built for; the "why" reads as a safety wrapper, not as a way to get more out of a strong model.
**Change:** Add a `## Built for frontier models` section between Why and How it works: the harness exists to get the most out of an AGI-class model and to reach the harder problems a coding agent skips; works best with the most capable model available in Claude Code, developed and self-run on Claude Fable 5.1. plugin.json description gains the same claim in one clause. Name no model the plugin cannot run: it is a Claude Code plugin, so only models available in Claude Code are named.
**Acceptance:** WHEN README.md is read THEN it SHALL contain a section headed `## Built for frontier models` that names Claude Fable 5.1, and `grep -c 'Fable 5.1' README.md` SHALL print 1.
**Verify:** `grep -c '^## Built for frontier models' README.md` → 1; `python3 -m unittest discover -s tests` → OK.
**Must not:** touch skills/, scripts/, or any test; claim compatibility with a model Claude Code cannot run.

### S-29 — HANDOFF never chooses a stopped hypothesis
**PR:** one.
**Depends on:** S-20.
**Files:** `scripts/report.py`, `tests/test_report.py`, `skills/research-council/references/report.md`.
**Today:** `handoff()` ranks every hypothesis whose status is not `refuted`, so one stopped by `rank.py stop` (S-20) can print as `Chosen:` (bug 10).
**Change:** Chosen is the highest-rated hypothesis with status `open`. Stopped and refuted rows stay in the Elo table with their status. report.md's Chosen approach row says `open`.
...
| S-25 | done 2026-09-09 (PR #19) | The proposed acceptance was untestable: `grep -c 'fixed PR'` printed 5 before and after because row 2 already carried the PR text in the wrong cell (the self-run read the table by eye, not by column). A self-proposed step gets its acceptance re-checked against the live file before it is accepted; here that turned a count into a last-cell grep that went 0 to 1. Docs only, no test. |
| S-24 | done 2026-09-09 (PR #20) | The snapshot has to exclude the Supervisor's own files or the fence trips on itself: `journal.jsonl` changes between snapshot and check whenever the Supervisor logs, and `fence/` grows with every role, so both are skipped and council.md says so. Gaps decided: a removed file is a violation too (`removed`, not `wrote`), since a role deleting evidence is worse than one adding a note; one journal note per check naming every file, not one per file; unknown role and missing snapshot are exit 1, not 2, so a forgotten snapshot never reads as a clean spawn. The council.md test first sliced from `## After every spawn` and missed the snapshot command that lives in the `Every spawn` block above; slice widened. Comparison off fails 7 tests, not one: every violation test rides on the same loop. 12 tests, 193 total. |
| S-18 | done 2026-09-09 (PR #21) | Shipped ahead of S-17 because the self-run hit bug 6 six times and bug 5 never. The supersede record is a second line shape in the same file, so `read()` now folds later records onto the claim and `add()` keeps numbering from claim lines only; the alternative (a `status` edit on the original line) would have broken the must-not. Gaps decided: a claim is superseded once, the replacement must carry evidence (an unverified claim cannot retire a verified one), self-supersede is refused, chains are allowed, and a superseded unverified claim shows under Superseded only. The section text is FIXED so the traceability test still passes with the reason copied verbatim. Removing the evidence check fails two tests, not one, because the CLI exit-code test uses the same refusal; truncating the file fails six. 11 tests, 204 total. |
| S-26 | done 2026-09-09 (PR #22) | The GIF had to be reproducible or the test could not say the committed file matches the source, so randomness is seeded per phase (mulberry32) and the browser and node share one Loop state machine; pose() has side effects (dig spawns dirt), so render() computes it exactly once per frame. GIF89a with a hand-written LZW encoder is about 60 lines of stdlib; PIL decoded all 122 frames on the first try. The first roundtrip test used 5k repetitive bytes and never reached code width 12, so breaking the width step failed only the GIF-is-current test: the test now feeds 40k random indices, fills the 4096 table and forces a clear, and the break fails both. Node is dev-only: the test that needs it skips, the GIF is committed. "WOO!" is a pixel glyph, not canvas text, so the GIF and the page draw the same thing. Shading is two rim bands plus cast shadows under head, paper and mound. |
| S-27 | done 2026-09-09 (PR #23) | Repo description and topics are not files, so the register step names the `gh repo edit` command and the verify is `gh repo view`. Install line uses `claude --plugin-dir` because the repo has no marketplace.json; adding one is a later step if listing matters. LICENSE was missing while plugin.json already said MIT. The acceptance grep was written as `claude-code` (the topic slug) but the README says "Claude Code"; the post-merge check printed 0, so the sentence was corrected to a case-insensitive `claude code` (prints 2) in PR #24. |
| S-28 | done 2026-09-09 (PR #25) | Copy only. The ask named a second model; left out because a Claude Code plugin cannot run on it and the README must not claim what cannot be verified (memo O-20). |
| S-17 | done 2026-09-09 (PR #26) | Disputed sits between What we found and Unverified and only takes claims that have evidence: a blocked unverified claim stays under Unverified and a blocked superseded claim under Superseded, because both are already not findings and one claim must appear in exactly one section. `blocking` must be the JSON `true`, not truthy, so a string "true" from a sloppy Reflection reply counts as malformed rather than as a block. Missing and malformed files get two different fixed lines: a missing file means Reflection never ran, which the reader should know; a bad file means the Supervisor saved something that is not objections JSON. Disputed claims also drop out of How sure, since that section lists findings. Fixture carries one non-blocking objection so every existing test exercises the file-present path; the acceptance test adds the blocking one. Break tests: skipping the objections read fails 3; leaving disputed claims in What we found fails 1; the first `is True` check survived a truthy break because no test sent a string, so a string `"true"` now raises and is covered by its own test (fails 1). The guard was then lost once to `git checkout --` on an uncommitted edit, the S-1 lesson again; re-applied and amended. 8 tests, 216 total. |
| S-19 | done 2026-09-10 (PR #27) | Beat is computed from `comparisons.jsonl` only: the highest-rated hypothesis the chosen one has a recorded win over; no win prints the one fixed line, which also covers the single-hypothesis case, so the old "only one hypothesis stands" line went. Fixture gained the one comparison its ratings already implied (H2 over H1). Break tests: restore `ranked[1]` fails 2 (acceptance + fixed line); count losses as wins survived the first test because every opponent was also a loser, so the test was rewritten around a loss to a higher-rated rival and then fails exactly 1. `git checkout --` after the commit this time; nothing lost. 28 report tests, 219 total. |
| S-20 | done 2026-09-10 (PR #28) | `stop` is one function, refusals before any write, same pattern as `record`; `pair` needed no change because eligibility was already `status == open`, so the acceptance test guards that line against a future loosening. Recommendation line format decided: `continue | stop — <reason>; stop: H1, H4`, where the first word is about stage 2 and the `stop:` list is about hypotheses; meta-review.md says so. A test greps every `agents/*.md` for `rank.py stop` to keep the command out of every role. Break tests: pair ignores status, stop skips the open check, stop resets elo each fail exactly 1. Found bug 10 while writing it: `report.py` handoff still picks Chosen from every non-refuted hypothesis, so a stopped one can be Chosen; logged and queued as S-29. 21 rank tests, 224 total. All 20 v1 steps merged. |
| S-29 | done 2026-09-10 (PR #30) | One-line fix: the Chosen filter now reads `status == rank.ELIGIBLE_STATUS` instead of `!= refuted`, so report.py shares rank.py's definition of eligible rather than keeping its own. `beaten()` left alone: a recorded win over a rival that was stopped later is still a recorded win, and the Beat line reads comparisons.jsonl, not status. Acceptance test uses `rank.stop` on the fixture's top-rated H2 and checks Chosen: H1, the fixed Beat line (H1 never won a pair), and the `stopped` row still in the table. Break tests: skip only refuted → exactly 1 fails; drop the filter entirely → exactly 1 fails. Bug 10 closed. 29 report tests, 225 total. |
| S-30 | done 2026-09-10 (PR #32) | Asked as "make the whole thing for general-intelligence AI models that can search and do a lot"; grilled to docs-only with the existing term `AGI-class model`, tools untouched, name still parked (O-19). The honest finding was that S-28 had put the claim only where users read (README, plugin.json) and nowhere the model reads: spec, both SKILL.md preambles, four role files. One paragraph at each, same term, and evidence.py already listed `web` so the capability sentence names what exists. Test guards the phrase in all six entry points and that no role gained Web or Bash; the second guard overlaps S-6's known-tool-names test, so that break fails 2. Other two breaks fail exactly 1. 3 tests, 228 total. |
| S-31 | done 2026-09-10 (PR #34) | Picked on a bare `next` with an empty queue, as the safest open item (memo O-05, owner Claude). One workflow, no install step, both skill validators after the suite; the node-only GIF test skips on the runner by design. Acceptance checked on the PR itself: `unittest (3.11)` and `unittest (3.12)` both pass, first CI run ever on the repo; a third check, CodeRabbit, is a pre-installed GitHub app, not this step. The old `sleep 90; gh pr checks` wait is blocked by the harness, so waiting for CI is a background `until` loop on `gh pr checks --json bucket`. Three breaks, one fail each. 4 tests, 232 total. "Still no CI" ends here. |
...
**Change:** Add prepare/accept/cancel commands. Prepare reserves one worker launch under the user budget, returns only the role's input and return-only instructions, and binds a request id to the frozen goal and input snapshot. Accept checks the request, unchanged input, schema, identifiers, and fence before writing only the role's named output; rejects replay, stale replies, unknown claims, or rating/goal edits. Ranking receives only its blinded pair and evidence/claims. The host must supply a genuinely read-only worker; the script itself never spawns a model or executes returned text. Serialize controller changes, record lock ownership for manual crash diagnosis, check journal symlinks, and atomically replace generated JSON/Markdown outputs after encoding. Do not automatically reclaim locks. Correct the role authority and violation wording.
...
**Today:** The four Upgrade status rows (S-32, S-35, S-33, S-34) still say they are only locally verified and unmerged, and README's Status paragraph still says the 0.2.0 candidate has not been published or merged. All four are on main: 7c71c3f (#36), 91e83f5 (#37), 5c22b4f (#38), cfde0ee (#41). Bug 22.
...
**Acceptance:** WHEN `docs/spec-v1.md`'s Upgrade status table is read THEN each of the S-32, S-35, S-33 and S-34 rows SHALL start `done` with a PR number, and WHEN `grep -c "not published, merged" README.md` is run THEN it SHALL print 0.
**Verify:** `grep -cE "^\| S-3[2345] \| done" docs/spec-v1.md` → 4; `grep -c "not published, merged" README.md` → 0; `python3 -m unittest discover -s tests` still passes.
...
| S-46 | merged 2026-09-21 (PR #TBD) | The four Upgrade rows and the README Status paragraph said the 0.2.0 chain was unmerged after all four were already on main (bug 22). Fixed by naming each row's real merge PR (S-32 #36, S-35 #37, S-33 #38, S-34 #41) and rewriting the README sentence the same way. The step's own original Acceptance line was self-referential: it grepped `docs/spec-v1.md` for the exact phrase its own Today and Acceptance text had to quote, so the count could never reach 0 while the step's record of itself existed. Reworded Today to paraphrase instead of quote, and Acceptance/Verify to read the table directly (`grep -cE "^\| S-3[2345] \| done"` → 4) instead of asserting a phrase's absence from the whole file. Docs only; `python3 -m unittest discover -s tests` still 492 OK. |
