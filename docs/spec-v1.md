...
| S-27 | done 2026-09-09 (PR #23) | Repo description and topics are not files, so the register step names the `gh repo edit` command and the verify is `gh repo view`. Install line uses `claude --plugin-dir` because the repo has no marketplace.json; adding one is a later step if listing matters. LICENSE was missing while plugin.json already said MIT. |
