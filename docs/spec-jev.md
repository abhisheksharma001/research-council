...
- BAML as a dependency (D-13). Its one useful idea, reading a loosely formatted reply against a
  declared schema, shipped without it as tolerant readers in S-65 and S-66; S-57's Status row
  says which parts did not.
...
### S-70 — the register says how much of S-57 shipped

**PR:** one (docs only).
**Depends on:** S-65, S-66.
**Research:** none.
**Files:** `docs/spec-jev.md`.
**Today:** S-57 has no Status row, and S-69's row reports it as delivered by S-65 and S-66. Read
against the code it is half delivered. Shipped: a Markdown code fence is stripped and the reply
parsed with `strict=False` (`scripts/council.py`, S-65); the meta-review recommendation is compared
case-folded (`scripts/council.py:264`, S-65); the ranking winner is resolved case-folded to its
`SCORE` key (`scripts/rank.py:147`, S-66). Not shipped: `scripts/done.py:59` still compares a
finding's severity exactly against `("blocking", "advisory")`. Contradicted: coercing the strings
"true" and "false" to booleans is the opposite of S-48, which pinned `scripts/report.py:116`
refusing `"blocking": "true"` with a named reason; `scripts/council.py` refuses it too. The
"Deliberately not here" line on BAML still says the idea is reimplemented "as S-57 if a step needs
it", which no longer says where it went.
**Change:** add an S-57 Status row stating the three parts: shipped in S-65 and S-66 without a
`repair.py`, the boolean coercion dropped because S-48 decided the opposite, and the severity
case-fold parked until S-44's dry run shows a Reviewer writing a miscased severity — after S-49 such
a file exits 1 before any test run or metered action, so unlike the council and rank cases a
refusal costs a retry, not a reservation. Reword the BAML line to name S-65 and S-66. The S-57
block itself stays as written; it is the record of what was asked.
**Acceptance:** WHEN `docs/spec-jev.md`'s Status table is read THEN it SHALL hold exactly one S-57
row naming S-65, S-66, S-48 and `scripts/done.py`, and WHEN the "Deliberately not here" list is read
THEN the BAML line SHALL name where its idea shipped.
**Verify:** `grep -c '^| S-57 |' docs/spec-jev.md` → 1; `python3 -m unittest discover -s tests` → OK
(the backticked-path test covers every new path).
**Must not:** change any script or test; mark S-57 done; build the severity case-fold here; edit
the S-57 block or any other step's row, except the one sentence in S-69's row that called S-57
delivered, which the working standard requires correcting where it was made.

### S-71 — both caps tables name the kinds budget.py counts

**PR:** one.
**Depends on:** S-51.
**Research:** none.
**Files:** `skills/code-writer-council/references/task.md`, `tests/test_budget.py`, `docs/spec-jev.md`.
**Today:** `scripts/budget.py` counts `max_actions` as journal lines whose kind is in
`journal.ACTION_KINDS`, which holds `judge` since S-51, for a task run as for a research run.
`skills/research-council/references/budget.md:20` lists judge; `skills/code-writer-council/references/task.md:27`
does not. S-51's row said the task table "stays true while the code battery is parked"; it does
not, because the counter is shared and a `journal.py add --kind judge` line on a task run counts.
No test ties either table to the code, so the next kind added drifts the same way.
**Change:** add `judge` to the task.md row. Add one test that reads the `max_actions` row of both
tables and requires the listed kinds to equal `journal.ACTION_KINDS`, plus a check that each row
was found, so a renamed row cannot pass silently. Correct S-51's sentence in its own row.
**Acceptance:** WHEN either caps table's `max_actions` row names a kind set different from
`journal.ACTION_KINDS` THEN `python3 -m unittest tests.test_budget` SHALL fail naming the file.
**Verify:** `python3 -m unittest discover -s tests` → OK; delete `judge` from task.md → one
failure naming task.md; add a kind to `journal.KINDS` → one failure per table.
...
| S-57 | partly done by S-65 (PR #65) and S-66 (PR #66); remainder parked until S-44 | Registered by S-70 because the step had no row and S-69's row called it delivered, which the code does not bear out. Of its four parts, two shipped without a `repair.py`: S-65 strips one Markdown code fence and parses with `strict=False` (`scripts/council.py:336` and `:347`) and compares the meta-review recommendation case-folded (`scripts/council.py:264`); S-66 resolves the ranking winner case-folded to its `SCORE` key (`scripts/rank.py:147`). Neither needed a normalisation pass or a journal note, because a tolerant reader leaves the stored bytes as sent. The third part is dropped: coercing the strings "true" and "false" to booleans is the opposite of what S-48 decided and test-pinned, `scripts/report.py:116` refusing `"blocking": "true"` with a named reason, and a review whose blocking flag is a string is exactly the case that decision exists for. The fourth is not built: `scripts/done.py:59` still compares a finding's severity exactly. It is parked, not dropped, until S-44's dry run shows a Reviewer writing a miscased severity, because since S-49 such a file exits 1 before any test run or metered action, so a refusal there costs one retry where the council and rank refusals cost a spawn reservation. Bug 28, which S-57 was written to fix, is closed in full by S-65 and S-66 regardless. |
...
| S-69 | done 2026-09-22 (PR #69) | The rule had to be discovered from the rows rather than imposed on them: nine of thirty state cells failed, and they failed in three different shapes — a bare step id (`S-17`), `fixed locally` written before the merge, and `(PR pending)` written the day of it — which is why the test asks for `PR #<number>` or an explicit `open`/`queued` rather than for any particular sentence. Every one of the nine was read off the current code before its cell was touched, and that is the step, not the typing: S-65's lesson says a row naming a step is not evidence the fix landed, and the opposite is just as true — `fixed locally` was not evidence it had not. All nine had, so every correction states work already on main and none of them changed a verdict. The two tables have six and four columns, so the test reads the last cell of each row and tells a header or separator apart by its own shape; with the header skip deleted, `state` itself fails the rule, which is what break 3 shows. A parser that finds nothing would pass the real assertion silently, so the row count is its own test — break 4 is the only reason that second test exists. Found on the way, twice: `tests/test_bug_log.py` was backticked in the step block before it was tracked, and S-67's path test refused the commit until `git add` ran, which is the convention working from the other side; and `S-57` has no Status row at all although S-65 and S-66 between them closed bug 28, which it was written to fix; S-70 found on reading the code that they delivered half of what it asked for, and gave it a row saying which half. That is queued, not fixed here. Suite 519 -> 521. |
| S-70 | done 2026-09-22 (PR #70) | The housekeeping item was written as "mark S-57 superseded", and doing exactly that would have put a false row in the register: reading each of S-57's four parts against the code found two shipped, one contradicted by a decision made two steps before it, and one never built. The queue entry had been copied forward from S-69's own sentence, so the error had already propagated once — from a Status row, into the memo, into the next step's plan — without anyone reading `scripts/done.py`. The honest state needed an existing word rather than a new one: `parked`, as S-56 already uses, says the remainder waits on evidence rather than on a decision nobody has made. A test that every declared step carries a Status row was measured and not written: `docs/spec-v1.md` has rows only for shipped steps, so the rule would have been a new convention imposed on three registers, not an existing one enforced. The one break available on a docs step is the cited paths, and S-67's test catches a wrong one. Suite unchanged at 521. |
| S-71 | done 2026-09-22 (PR #71) | A queued doc fix carried its own reason for waiting, "stays true while the code battery is parked", and the reason was false: `scripts/budget.py` has one `max_actions` counter for task and research runs, so the task table had been incomplete since S-51 whether or not judge.py ever runs on a task. judge.py refusing task runs made it rare, not true. The fix is one word; the test is the lasting part, because it reads the rule from `journal.ACTION_KINDS` instead of from either document, and a found-exactly-one-row check keeps a renamed row from passing as empty. Proof: dropping judge from task.md fails once naming task.md; adding a kind to `journal.KINDS` fails once per table; renaming budget.md's row fails on the row count. Suite 521 → 522. |
| S-53 | done 2026-09-22 (PR #72); `fitted` stays empty | The step did its job by refusing to finish: the first live calibration (218 calls, $0.0085, `jev-1.13.0`) reached TPR 0.85 on unsupported claims at TNR 0.62 on supported ones on the held-out split, with 13 and 8 cases, so the acceptance's second clause held and nothing was fitted. The largest error was not Jev's but the exporter's own labels, and the train-only misses packet is what showed it: every `contradicted` miss was a counterexample claim, which agents/reflection.md lets cite any record on file while the state carries only the cited excerpts, and every `inferred` false alarm on a clean claim was one Reflection had accepted as `inferred`, labelled the opposite. Fixing the two rules against the rubric, then re-scoring the same cached answers at no cost, moved TPR from 0.58 to 0.85, which is a warning worth keeping: a calibration is only as good as the definition its labels share with its questions, and that is checked by reading misses, not by reading the headline number. The one question rewrite the step allows won on train loss for both questions and lost one eval error on each, and the tool flagged it as overfit itself, so the baseline wording stays; a referee that can say no is the point of having one. All 30 invented hard negatives were flagged, so the misses live in real Reflection labels, not textbook cases. The case count is the binding constraint: 33 claims were never read by Reflection and 22 were stopped by the phone rule across the three runs outside this repository, which S-68 measured on only two. `GET /v1/models` no longer lists the pinned id, yet every response carried it. Break 1 fails four tests because every positive depends on the fence match. Suite 522 -> 529. |
| S-72 | done 2026-09-22 (PR #73) | Opened as "measure the phone stops on the three runs outside this repository", which S-53's row named as the cheapest way to recover cases, and the measurement moved the step somewhere else entirely. The first count was wrong and wrong in my own favour: `build_claim_state` returns `(state, cited)`, the measuring script dumped the whole tuple, and the 88 stopped claims it reported were mostly digit runs inside the evidence records' `sha256` fields, which the guard never sees. Dumping element zero gives 33 of 191, and reading the spans rather than the count is again what gave the fix — 27 of the 33 match only inside a `uri`, as x.com status ids of nineteen digits and an App Store app id of ten. So the defect was never the pattern S-68 had already narrowed, it was a field in the state: no claim question asks who published the page. Removing it beats narrowing the pattern inside a URL, which was the other candidate and was rejected for a reason worth keeping — a URL is untrusted fetched text, `tel:+15551234567` is a legal one, and weakening a guard to recover cases is the trade this seam exists to refuse. Nothing else depended on it: `build_evidence_state` still sends the page uri because `host_rule` reads it, and no other EGRESS pattern fired on a uri in any of the 191 claims, measured before the edit. 33 stops become 7 and the state is 11% shorter. The S-53 calibration note was corrected where it was written rather than in this row: every rate on that page was measured on a state shape that no longer exists, so S-54 needs fresh calls, not just more cases. Break 2 fails eight tests rather than one because the evidence state's uri carries the host rule as well as the guard. Suite 529 -> 531. |
