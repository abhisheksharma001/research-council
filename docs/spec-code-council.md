...
| S-40 | done 2026-09-21 (PR #47) | The spec carried two shapes for a blocking finding (`blocking: true` here, `severity` in S-41); done.py reads `severity: "blocking"` because that is what the Reviewer will emit, so one schema exists. Nobody was named to write resolutions.jsonl, and a hand-typed sha would be the model composing the proof rule 1 forbids; `done.py resolve` writes the line and computes the diff sha itself, refusing an id no review file or thinker.json holds. The sha hashes `git diff --binary` plus every untracked file (S-38), and because a test run writes `__pycache__` or coverage files, done.py hashes before and after the run and refuses a diff the tests changed: the workspace must ignore those files. `references/tiers.md` said a tier 2 or 3 diff with no reviewer is not done, but nothing enforced it; done.py now requires a review file for tier 2 or 3 and thinker.json when max_subagents allows the Thinker. Two Reviewers both counting from R-1 would collide, so an id in two review files is exit 1: S-41 must give each lens its own id prefix. A shell test command leaves grandchildren alive on timeout unless the process group is killed. Suite 413 tests. |
| S-41 | done 2026-09-21 (PR #48) | The Reviewer's ids carry a lens prefix (`RA-<n>` correctness+security, `RB-<n>` scope+erosion) because done.py exits 1 on an id shared by two review files and two parallel Reviewers would both start at R-1. The acceptance asked for exactly one failing test on Write or Bash: Write is guarded here, Bash by S-30's repo-wide `test_no_role_gained_a_web_or_shell_tool`, so this step adds no second Bash check (a duplicate guard would have made the count two, the S-36 lesson). The fence's gate was goal.json only, as S-37 predicted for every shared script; it now takes task.json too. The review stage saves each reply only after `fence.py check`, because a review-<n>.json written before the check is itself a violation. `diff.patch` joined the run folder's file list. zsh does not word-split an unquoted variable, so a test-module list held in a shell variable ran as one mangled name and every proof-by-breaking run errored; spell the modules out. Suite 427 tests on the branch. |
| S-42 | done 2026-09-21 (PR #55) | The step's file list missed `tests/test_who_runs_this.py` again: it hard-counts entry points (agents plus skills), so a sixth agent makes the count 9 — S-36 predicted this for every step that adds an agent, and S-41 hit it too; treat it as part of any agent step's file list. The step text names no fence, but `scripts/fence.py` already carried the `code-thinker` role with an empty allowed set from S-41, so the spawn is fenced like the Reviewer's and an unfenced spawn stays impossible in either council; `budget.py` is read-only and journal.jsonl is fence-skipped, so "write nothing into the run folder while it is out" is a rule the stage can actually keep. The write stage gates on `expected_small: false` and `max_subagents >= 2`, which is exactly done.py's own demand (`tier >= 2 and max_subagents >= 2` needs thinker.json), so the two cannot disagree; the misestimate branch exists because a task that expected small and lands at tier 2 would otherwise be NOT DONE with no way to fix it. The acceptance says Write, Edit or Bash, but Bash is not re-checked here: S-30's repo-wide `test_no_role_gained_a_web_or_shell_tool` covers it, and a duplicate guard would have made that break two failures instead of one (S-41's lesson). Self-review caught a false sentence: the Thinker has no Bash, so it cannot read `start_commit` — it reads a working tree the Supervisor is editing under it, and the role file now says so. Seven breaks, one failing test each. Suite 485. |
| S-43 | done 2026-09-21 (PR #56) | First step in this register whose file list was complete as written; `tests/test_who_runs_this.py` counts entry points, and this step adds none. The ordering test anchors on literal command strings, so the wording of every other stage has to keep out of their way: stage 3 says a split gives each task "its own done.py run", never "done.py check", or `t.find("done.py check")` would return stage 3's index and the order test would pass a broken file. A block-slicing test must survive the very reorder it exists to catch: slicing the Fix stage as Fix→Done gives an empty block the moment Done moves above Fix, so the one-line reorder would have failed two tests instead of one; Fix→Reply and Done→Reply hold in both orders, which is what makes the acceptance's "exactly one test" true. Self-review caught a quoted half-line: `goal.IGNORE_WARNING` reads `warning: AGI_Research/ is not ignored by <gitignore>; the run folder holds client data`, and quoting only the first clause in prose reads as the whole printed line while dropping the reason the warning exists. Stage 6 sends a re-review back through stage 5 rather than saying "spawn the Reviewer again", because stage 5 carries the budget check every other spawn passes. No `references/*.md` changed: task.md, tiers.md, done.md and deps.md already hold the detail and the stages point at them, which is what kept the body at 147 lines of 500. Seven breaks, one failing test each. Suite 492. |
